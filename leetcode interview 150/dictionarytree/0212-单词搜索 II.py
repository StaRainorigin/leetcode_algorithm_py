...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(cur: Optional[TrieNode], i: int, j: int):
            if i < 0 or i >= n or j < 0 or j >= m or board[i][j] not in cur.sons:
                return
            c = board[i][j]
            cur = cur.sons[c]
            if cur.word:
    ...
    #         if index >= len(word):
    #             return True
    #         if i < 0 or i >= n or j < 0 or j >= m or (i, j) in visited or board[i][j] != word[index]:
    #             return False
    #         else:
    #             visited.add((i, j))
    #             index += 1
    #             is_find = dfs(i, j+1, word, index, visited) or dfs(i+1, j, word, index, visited) or dfs(i, j-1, word, index, visited) or dfs(i-1, j, word, index, visited)
    #             if is_find:
    #                 return True
    #             else:
    #                 visited.remove((i, j))
    #     ans = []
    #     for word in words:
    #         if not word or word[0] not in alphabet:
    #             continue
    #         for i, j in alphabet[word[0]]:
    #             visited = set()
